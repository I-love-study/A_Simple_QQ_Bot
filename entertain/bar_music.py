...
async def bar_music(app: Ariadne, group: Group, sparkle: Sparkle):
	if sparkle.para.result.asDisplay().strip() == '':
		await app.sendGroupMessage(group, MessageChain.create('点啥歌？'))
		return
	search_data = await Netease.search(sparkle.para.result.asDisplay().strip())
	try:
